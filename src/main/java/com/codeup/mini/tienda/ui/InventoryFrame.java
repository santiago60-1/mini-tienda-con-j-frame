...
import com.codeup.mini.tienda.domain.Appliance;
import com.codeup.mini.tienda.domain.Food;
...
public class InventoryFrame extends javax.swing.JFrame {
    ...
    private void updateTable() {
        ...
        ArrayList<Product> lista = inventory.getProducts();
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
               ...
               try {
            String categoriaTexto = lblCategory.getText().trim();
            String name = lblProduct.getText().trim();
            double price = Double.parseDouble(lblPryce.getText().trim());
            int stock = Integer.parseInt(lblStock.getText().trim());

            // Validaciones
            if (categoriaTexto.isEmpty() || name.isEmpty()) {
                javax.swing.JOptionPane.showMessageDialog(this, "Debe completar todos los campos");
                return;
            }
            if (price <= 0 || stock <= 0) {
                javax.swing.JOptionPane.showMessageDialog(this, "El precio o el stock no pueden ser cero o negativos");
                return;
            }

            // Verificar duplicado en el inventario local
            ...
            Product nuevo;
            if (categoriaTexto.equalsIgnoreCase("Electrodomestico") || categoriaTexto.equalsIgnoreCase("Electrodoméstico")) {
                nuevo = new Appliance(name, price, stock);
            } else if (categoriaTexto.equalsIgnoreCase("Alimento") || categoriaTexto.equalsIgnoreCase("Comida")) {
                nuevo = new Food(name, price, stock);
            } else {
            ...
            javax.swing.JOptionPane.showMessageDialog(this, "Producto agregado correctamente (memoria)");
            updateTable();

            // Persistir en la base de datos usando los DAOs corregidos
            CategoryDaoImpl categoryDao = new CategoryDaoImpl();
            ProductsDAOimple productDao = new ProductsDAOimple();

            try {
                Category cat = categoryDao.searchByName(categoriaTexto);

                if (cat == null) {
                    Category nuevaCat = new Category(categoriaTexto);
                    // insert devuelve la Category con id en la versión corregida del DAO
                    try {
                        nuevaCat = categoryDao.insert(nuevaCat);
                    } catch (SQLException ex) {
                        // si insert falla, intentamos crear sin recuperar id (método create)
                        categoryDao.create(new Category(categoriaTexto));
                    }
                }

                // Insertar el producto usando la firma actual create(Product)
                try {
                    productDao.create(nuevo); // usa el método que ya tienes
                    javax.swing.JOptionPane.showMessageDialog(this, "✅ Producto guardado en la base de datos");
                } catch (SQLException ex) {
                    javax.swing.JOptionPane.showMessageDialog(this, "❌ Error al guardar producto en BD: " + ex.getMessage());
                }

            } catch (SQLException sqle) {
                javax.swing.JOptionPane.showMessageDialog(this, "Error de base de datos: " + sqle.getMessage());
                sqle.printStackTrace();
            }

        } catch (NumberFormatException e) {
            javax.swing.JOptionPane.showMessageDialog(this, "Precio y stock deben ser numéricos");
        }
    ...
    try {
        // 1) Obtener el resumen del ticket que ya tienes
        String summary = inventory.generateTicket();
        javax.swing.JOptionPane.showMessageDialog(this, summary);

        // 2) Construir Invoice usando el summary como observaciones
        com.codeup.mini.tienda.domain.Invoice invoice = new com.codeup.mini.tienda.domain.Invoice();
        invoice.setObservaciones(summary);

        // 3) Intentar extraer un total desde el summary (busca la última aparición de un número con decimales)
        java.math.BigDecimal total = java.math.BigDecimal.ZERO;
        try {
            java.util.regex.Matcher m = java.util.regex.Pattern.compile("(\\d+(?:[\\.,]\\d{1,2})?)").matcher(summary);
            java.util.List<String> matches = new java.util.ArrayList<>();
            while (m.find()) matches.add(m.group(1));
            if (!matches.isEmpty()) {
                String candidate = matches.get(matches.size() - 1).replace(",", ".");
                total = new java.math.BigDecimal(candidate);
            }
        ...
        // 4) Crear un InvoiceItem resumen usando la clase del domain corregida
        com.codeup.mini.tienda.domain.InvoiceItem summaryItem =
            new com.codeup.mini.tienda.domain.InvoiceItem("Venta sesión", 1, total);
        invoice.addItem(summaryItem);

        // 5) Persistir transaccionalmente con InvoiceDaoImpl
        com.codeup.mini.tienda.dao.daoimpl.InvoiceDaoImpl invoiceDao = new com.codeup.mini.tienda.dao.daoimpl.InvoiceDaoImpl();
        try {
            int idFactura = invoiceDao.saveTicket(invoice);
            javax.swing.JOptionPane.showMessageDialog(this, "Factura guardada con id: " + idFactura);
        } catch (SQLException sqle) {
            javax.swing.JOptionPane.showMessageDialog(this, "Error guardando factura: " + sqle.getMessage());
            sqle.printStackTrace();
        }

        // 6) Cerrar la ventana / salir
        dispose();

    } catch (NumberFormatException e) {
        javax.swing.JOptionPane.showMessageDialog(this, "Error al generar el ticket: valores numéricos inválidos");
    } catch (Exception ex) {
    ...
    private int repositoryCategory(String categories) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }

    private int repositoryProduct(Product nuevo, int stock) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }
}
